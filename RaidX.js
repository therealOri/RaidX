    ...
    if (prefix.test(message.content)) {
        ...
        switch (parts[0]) {
            case 'ping':
                ...
                break
            case "status":
                sendMessage(message, '***Online | Version 1.1.2***')
                break
            case "invite":
                sendMessage(message, `https://discordapp.com/oauth2/authorize?client_id=${client.user.id}&scope=bot&permissions=8`)
                break
            case "help":
                sendMessage(message, '```-invite | Gives you the bots Invite, -status | Gives you the bots status and version, -kick @user kickes said user, | -ban @user bans said user, | -vote will let you vote for the bot!, More will be added in the future!```')
                break
            case "kick":
                if (message.channel.type !== "text") return
                if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("You do not have permission to kick users!")
                if (!message.guild.me.hasPermission("KICK_MEMBERS")) return message.channel.send("I don't have permission to kick users!")
            ...
            case "ban":
                ...
                if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send("I don't have permission to ban users!")
                if (message.members && message.mention.members.size < 1) return message.channel.send("Please mention someone!")
                let banned = []
                let notBanned = []
                for (let member of message.mentions.members.values()) {
                ...
                break
            case "vote":
                sendMessage(message, 'https://discordbots.org/bot/537126219095736351/vote')
                break
        }
    }
})
